  ...
  return (
    <div className="flex items-center gap-4">
      <div className="flex gap-4 text-xs text-gray-500">
        <button onClick={() => handleVote('like')} disabled={loading} className="hover:text-blue-400 transition-colors">👍 {likes}</button>
        <button onClick={() => handleVote('dislike')} disabled={loading} className="hover:text-red-400 transition-colors">👎 {dislikes}</button>
      </div>
      {showLogin && (
        <button onClick={() => navigate('/login')} className="text-xs text-[#e30613] hover:underline animate-pulse">
          로그인 필요
        </button>
      )}
    </div>
